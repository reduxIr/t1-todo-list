...
const TaskItem: React.FC<ITaskItem> = ({ task }) => {
  const clueEditRef = useRef<HTMLSpanElement | null>(null);
  const [isModalOpen, setIsModalOpen] = useState<Boolean>(false);
  const [editedTask, setEditedTask] = useState<Task>(task);
  const [isHovered, setIsHovered] = useState<Boolean>(false);
  
  useEffect(() => {
    if (isHovered && clueEditRef) {
      const typed = new Typed(clueEditRef.current, {
      strings: ['Click to edit'],
      typeSpeed: 80,  
      onComplete: () => {
  ...
  const handleSave = () => {
    if (!editedTask.title.trim() || !editedTask.category || !editedTask.priority || !editedTask.status) {
  ...
  return (
    ...
    <div>
      <div className="flex ">
        ...
        </button> : <div className="w-12 bg-white m-1"></div>
        }
      </div>
    ...
    {isHovered && 
      <div className="text-gray-400 font-[Orbitron] ml-1 text-sm">
        <hr className="w-30 mt-0.5 border-gray-400"/>
        <span ref={clueEditRef}></span>
      </div>
    }
